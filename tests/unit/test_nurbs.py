...
class TestNurbsCreate:
    def test_create_primitive_plane(self):
        surface = Nurbs.create("nurbsPlane", name="testPlane")
        ...
        assert surface.transform.name == "testPlane"

    def test_create_primitive_sphere(self):
        surface = Nurbs.create("sphere", name="testSphere")
        assert isinstance(surface, Nurbs)
        assert surface.transform.name == "testSphere"

    def test_create_node_surface(self):
        # cmds.createNode("nurbsSurface") returns the shape name directly
        surface = Nurbs.create("nurbsSurface", name="testSurfaceShape")
        assert isinstance(surface, Nurbs)
        assert surface.name == "testSurfaceShape"
