...
from tik.trigger.core.document import ActionNode, Document, SCHEMA_VERSION
...
def test_walk_paths_find_parent():
    doc = _doc()
    assert doc.paths() == ["import_model", "import_model/rename", "kinematics", "cleanup"]
...
def test_add_unique_names_and_index():
    ...
    assert doc.add(ActionNode("rename", "script"), parent="import_model") == "import_model/rename1"
...
def test_move_rename_remove_duplicate():
    ...
    assert doc.paths()[-1].startswith("import_model") or "import_model" in doc.paths()[-2]
...
def test_old_flat_session_converts():
    ...
    assert [node.type for node in doc.actions][:3] == ["master", "import_asset", "kinematics"]
    kin = doc.find("kinematics1")
    assert kin.settings["guide_roots"] == ["base_c"]
    assert kin.children == [] and kin.enabled is True
    assert Document.from_dict([{"name": "a", "type": "script", "data": {"x": 1}}]).actions[0].settings == {"x": 1}
...
def test_versioning(tmp_path):
    ...
    assert versioning.resolve(tmp_path / "hero_v001.tr", "latest").name == "hero_v005.tr"
