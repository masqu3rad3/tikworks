...
def test_create_joint_with_position():
    ...
    j = Joint.create(name='posJoint', position=(5, 10, 15))
    translate = cmds.getAttr(f'{j.name}.translate')[0]
...
def test_create_joint_with_orientation():
    ...
    j = Joint.create(name='orientedJoint', orientation=(15, 25, 35))
    jo = cmds.getAttr(f'{j.name}.jointOrient')[0]
...
def test_create_joint_with_scale():
    ...
    j = Joint.create(name='scaledJoint', scale=(2, 3, 4))
    scale = cmds.getAttr(f'{j.name}.scale')[0]
    assert scale == pytest.approx((2, 3, 4), abs=1e-6)


def test_create_joint_with_radius():
    """Test creating a joint with radius."""
    j = Joint.create(name='radiusJoint', radius=5.0)
    assert j.radius == pytest.approx(5.0, abs=1e-6)


def test_joint_radius_property():
    """Test getting and setting joint radius."""
    j = Joint.create(name='radiusPropJoint')

    # Get default radius
    default_radius = j.radius
    assert isinstance(default_radius, float)

    # Set new radius
    j.radius = 3.5
    assert j.radius == pytest.approx(3.5, abs=1e-6)
    assert cmds.getAttr(f'{j.name}.radius') == pytest.approx(3.5, abs=1e-6)
