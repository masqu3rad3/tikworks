...
def wired_designer(qapp):
    ...
    session = Session()
    original = ModuleEntry(instance_id="old-id", module_type="fkchain", name="spine", side="C")
...
def test_a_rejected_dialog_leaves_the_session_untouched(wired_designer, monkeypatch):
    ...
    assert [entry.instance_id for entry in session.document.guides.modules] == ["old-id"]
...
def test_an_accepted_dialog_replaces_the_session_modules(wired_designer, monkeypatch):
    ...
    assert [entry.instance_id for entry in session.document.guides.modules] == ["new-id"]
