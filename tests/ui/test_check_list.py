...
# ------------------------------------------------------------- groups
def test_ticking_a_group_ticks_every_member():
    editor = _editor()
    editor.set_groups([("L_fingers", ["id_larm", "id_lwing"])])
    editor.tick_group("L_fingers", True)
    assert set(editor.value()) == {"id_larm", "id_lwing"}


def test_unticking_a_group_leaves_other_ticks_alone():
    editor = _editor(value=["id_spine", "id_larm", "id_lwing"])
    editor.set_groups([("L_fingers", ["id_larm", "id_lwing"])])
    editor.tick_group("L_fingers", False)
    assert editor.value() == ["id_spine"]


def test_the_stored_value_never_holds_a_group_name():
    """A group is a way to tick several boxes, never a value the list stores."""
    editor = _editor()
    editor.set_groups([("L_fingers", ["id_larm", "id_lwing"])])
    editor.tick_group("L_fingers", True)
...
def test_a_group_adds_no_rows_to_the_list():
    """A checkable row that is not a value is a row every code path has to
    special-case; the bulk edit lives in the menu instead."""
    editor = _editor()
    before = _labels(editor)
    editor.set_groups([("L_fingers", ["id_larm", "id_lwing"])])
    assert _labels(editor) == before


def test_the_menu_offers_a_bulk_edit_per_group():
    editor = _editor()
    editor.set_groups([("L_fingers", ["id_larm", "id_lwing"])])
    texts = [action.text() for action in editor.context_menu().actions()]
    assert any("L_fingers" in text for text in texts)


def test_no_groups_means_no_group_entries_in_the_menu():
    editor = _editor()
    before = [action.text() for action in editor.context_menu().actions()]
    editor.set_groups([])
    assert [action.text() for action in editor.context_menu().actions()] == before


def test_ticking_an_unknown_group_is_a_no_op():
    editor = _editor(value=["id_spine"])
    editor.set_groups([("L_fingers", ["id_larm"])])
    editor.tick_group("nope", True)
    assert editor.value() == ["id_spine"]
