"""A plain cmds script which creates a test rig for comparison purposes."""
...
def create_test_rig():
    ...
    cmds.xform(main_geo, translation=[0, 5, 0])

    # Freeze transformations and delete history
    cmds.makeIdentity(main_geo, apply=True, t=1, r=1, s=1, n=0)
    cmds.delete(main_geo, constructionHistory=True)

    # ---------------------------------------------------------
    # Create Blendshape (Bulge)
    # ---------------------------------------------------------
    # Duplicate the cylinder
    target_geo = cmds.duplicate(main_geo, name='bulge_target')[0]

    # Create a non-linear flare deformer to make it "bulgy"
    flare_node, flare_handle = cmds.nonLinear(target_geo, type='flare')

    # Adjust flare attributes
    cmds.setAttr(f"{flare_node}.startFlareX", 1.8)
    cmds.setAttr(f"{flare_node}.startFlareZ", 1.8)
    cmds.setAttr(f"{flare_node}.endFlareX", 1.8)
    cmds.setAttr(f"{flare_node}.endFlareZ", 1.8)
    cmds.setAttr(f"{flare_node}.curve",
                 -0.8)  # Negative curve creates the bulge

    # Delete history on the target to bake the deformation
    cmds.delete(target_geo, constructionHistory=True)

    # Create Blendshape: Target -> Main
    bs_node = cmds.blendShape(target_geo, main_geo, name='cylinder_BS')[0]

    # Delete the target mesh as requested
    cmds.delete(target_geo)
    ...
    for i in range(joint_count):
        ...
        jnt = cmds.joint(p=(0, y_pos, 0), name=f"bind_{i}_JNT")
        bind_joints.append(jnt)

    # ---------------------------------------------------------
    # Skin Main Cylinder
    # ---------------------------------------------------------
    cmds.skinCluster(bind_joints, main_geo, toSelectedBones=True,
    ...
    ik_handle, effector, ik_curve = cmds.ikHandle(
    ...
    ik_curve = cmds.rename(ik_curve, 'spline_curve')
    ...
    for i, pos in enumerate(control_positions):
        ...
        cmds.select(clear=True)
        jnt = cmds.joint(p=(0, pos, 0), name=f"driver_{i}_JNT")
    ...
    for i, target_jnt in enumerate(control_joints):
        ...
        ctrl = cmds.circle(normal=(0, 1, 0), radius=2.5, name=f"ctrl_{i}_CRV")[
            0]

        # Match controller position to the joint
        cmds.matchTransform(ctrl, target_jnt, pos=True, rot=True)
        ...
        cmds.parentConstraint(ctrl, target_jnt, maintainOffset=True)
    ...
    # Master Controller
    # ---------------------------------------------------------
    master_ctrl = cmds.circle(normal=(0, 1, 0), radius=5, name="master_cont")[
        0]

    # Color the master controller yellow (override color 17) for visibility
    cmds.setAttr(f"{master_ctrl}.overrideEnabled", 1)
    cmds.setAttr(f"{master_ctrl}.overrideColor", 17)

    # Parent individual controllers under master_cont
    cmds.parent(controllers, master_ctrl)
    ...
    cmds.addAttr(master_ctrl, longName=attr_name, attributeType='float', min=0,
                 max=1, keyable=True)
    ...
    if cmds.objExists(bs_target_attr):
        cmds.connectAttr(f"{master_ctrl}.{attr_name}", bs_target_attr)
    else:
        cmds.error(
    ...
    rig_grp = cmds.group(empty=True, name="RIG_SYSTEM_GRP")
    geo_grp = cmds.group(empty=True, name="GEOMETRY_GRP")

    cmds.parent(main_geo, geo_grp)
    cmds.parent(bind_joints[0], rig_grp)
    cmds.parent(ik_handle, rig_grp)
    cmds.parent(ik_curve, rig_grp)

    # Group the driver joints
    driver_grp = cmds.group(control_joints, name="DRIVER_JOINTS_GRP")
    cmds.parent(driver_grp, rig_grp)
