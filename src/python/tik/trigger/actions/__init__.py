...
4. Optionally loads ui_definition.json and defaults.json
...
def _load_action_json(folder_path: Path, action_name: str) -> tuple[Optional[list], Optional[dict]]:
    """Load ui_definition.json and defaults.json from action folder.
    ...
    Returns:
        Tuple of (ui_definition, defaults) or (None, None) if files don't exist.
    """
    ui_definition = None
    defaults = None
    ...
    if defaults_path.exists():
        defaults = ConfigIO._load_json(defaults_path)
        logger.debug("Loaded defaults for %s", action_name)

    return ui_definition, defaults
...
def _register_action_from_folder(action_dir: Path) -> bool:
    ...
    ui_def_json, defaults_json = _load_action_json(action_dir, action_name)
