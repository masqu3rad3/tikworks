...
def build_reach(
    ...
    *,
    ...
    rest = tm.Transform.create(
        name=rig.name(name, "rest"), parent=rig.groups.rig.long_name
    )
    ...
    probe = tm.Transform.create(
        name=rig.name(name, "probe"), parent=rest_from.long_name
    )
    ...
    scaled = tm.create_node("multiplyDivide", name=rig.name(name, "scaleMultiply"))
    ...
    aim_point = tm.Transform.create(
        name=rig.name(name, "aimPoint"), parent=rest_from.long_name
    )
    scaled["output"] >> aim_point["translate"]

    angle = tm.AngleBetween.create(
        rest_direction, scaled["output"], name=rig.name(name, "angle")
    )
    ...
    blend = tm.MatrixBlend.create(
        rest, [frame.transform], [weight], name=rig.name(name, "blend")
    )
