...
- **Modules can be referenced** from another `.tr`: a `ModuleReference` in the guide document (never on the pipeline `reference` action -- actions are a sequence, modules are a set). Resolution inserts borrowed entries into the real `modules` list carrying runtime `origin`/`source`, so every guide-layer read and write works on them unchanged; `to_dict` skips them and emits each link's overrides by **diffing** against the source, which makes an override self-cleaning. Structure is upstream's word -- `enabled=False` leaves a module out, `remove` is refused.
