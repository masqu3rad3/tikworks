...
- **Status (August 2026):** workflow v2 — the `.tr` session is the rig (nested actions, references with overrides, runner), guides are a `.trg` asset, `Session`/`GuideScene` TD handlers, pipeline UI on the official theme, Guide Designer with two-way binding. Modules `base`/`fkchain`/`arm`; actions `import_asset`/`kinematics`/`script`/`reference`. Maya-only since the 2026-08-30 simplification pass.
- **Design specs:** `docs/superpowers/specs/2026-08-30-trigger-simplification-design.md` (layering and the `rig` object, authoritative), `2026-08-29-trigger-ui-v3-and-io-graph-design.md` (module I/O model), `2026-08-28-trigger-workflow-and-ui-design.md` (session blueprint), `2026-08-28-trigger-rebuild-design.md` (tik.maya constructs, fields); plans in `docs/superpowers/plans/`
