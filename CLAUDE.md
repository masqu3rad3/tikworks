...
- **One session document** (`.tr`, schema 7; the nested guide document is schema 3): guides + two ordered action lists (`actions` is the build list, `publish` the post-build one). The scene is a checkout of exactly one session at a time, stamped on the guide holder (`Session.capture_guides` / `checkout_guides`)
...
- `tests/unit/test_module_group_trigger.py` -- the pure record, the operations and shared-value derivation; `tests/unit/test_draw_state_trigger.py` -- what a group row shows; `tests/ui/test_group_tabs.py` -- the tab bar, the Common split and the tree rows; `tests/ui/test_graph_groups.py` -- the frame, the collapsed node, fan-in and the member picker; `tests/ui/test_stub_groups.py` -- the double matches the real surface; `tests/integration/trigger/test_module_groups_trigger.py` -- the scene verbs; `tests/integration/trigger/test_group_invariant_trigger.py` -- grouping never changes the rig
